...
-- Create the new COMMENT_REACTIONS table
CREATE TABLE IF NOT EXISTS COMMENT_REACTIONS (
    REACTION_ID INT NOT NULL AUTO_INCREMENT,
    COMMENT_ID INT NOT NULL,
    USER_ID INT NOT NULL,
    REACTION_TYPE VARCHAR(50) NOT NULL, -- 'yummy', 'delicious', 'tasty', 'love', 'amazing', 'like'
    REACTION_EMOJI VARCHAR(10) NOT NULL, -- Store the emoji for display
    CREATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UPDATED_AT TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (REACTION_ID),
    UNIQUE KEY unique_user_comment_reaction (COMMENT_ID, USER_ID), -- One reaction per user per comment
...
-- Create a view for easy comment reaction counting (similar to post reactions)
CREATE OR REPLACE VIEW COMMENT_REACTION_COUNTS AS
SELECT 
    c.COMMENT_ID,
    COALESCE(COUNT(cr.REACTION_ID), 0) as total_reactions,
    COALESCE(SUM(CASE WHEN cr.REACTION_TYPE = 'yummy' THEN 1 ELSE 0 END), 0) as yummy_count,
    COALESCE(SUM(CASE WHEN cr.REACTION_TYPE = 'delicious' THEN 1 ELSE 0 END), 0) as delicious_count,
    COALESCE(SUM(CASE WHEN cr.REACTION_TYPE = 'tasty' THEN 1 ELSE 0 END), 0) as tasty_count,
    COALESCE(SUM(CASE WHEN cr.REACTION_TYPE = 'love' THEN 1 ELSE 0 END), 0) as love_count,
    COALESCE(SUM(CASE WHEN cr.REACTION_TYPE = 'amazing' THEN 1 ELSE 0 END), 0) as amazing_count,
